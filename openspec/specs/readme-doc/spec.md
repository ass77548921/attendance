...
README SHALL 提供從零開始在本機啟動應用程式的完整步驟。

#### Scenario: 首次本地啟動
- **WHEN** 開發者按照 README 的本地啟動章節操作
- **THEN** 文件 SHALL 涵蓋前置需求（Java、Docker）、資料庫啟動、環境變數設定、Gradle 啟動指令
...
README SHALL 說明如何啟動應用後存取 Swagger UI 及 OpenAPI 文件，包含完整的存取步驟。

#### Scenario: 存取 Swagger UI
- **WHEN** 開發者依照 README 的 API 文件章節操作
- **THEN** 文件 SHALL 說明：先啟動應用（`./gradlew bootRun`），再以瀏覽器開啟 `http://localhost:8080/swagger-ui.html` 即可看到互動式 API 文件
