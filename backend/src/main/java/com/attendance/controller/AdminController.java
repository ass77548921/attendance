...
public class AdminController {
    ...
    public ResponseEntity<Page<AttendanceRecordResponse>> queryAttendance(
        ...
        @PatchMapping("/attendance/{id}/adjust")
        @Operation(summary = "手動修正出勤紀錄（需填寫原因）")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "修正成功"),
                        @ApiResponse(responseCode = "400", description = "輸入不合法"),
                        @ApiResponse(responseCode = "404", description = "紀錄不存在")
        })
        public ResponseEntity<AttendanceRecordResponse> adjustAttendance(
                        @PathVariable Long id,
                        @AuthenticationPrincipal User reviewer,
                        @Valid @RequestBody AdminAdjustAttendanceRequest request) {
                return ResponseEntity.ok(attendanceService.adminAdjustAttendance(id, request, reviewer));
        }

        @GetMapping("/attendance/{id}/adjustments")
        @Operation(summary = "查詢出勤修正歷史")
        @ApiResponse(responseCode = "200", description = "查詢成功")
        public ResponseEntity<java.util.List<AttendanceAdjustmentAuditResponse>> adjustmentHistory(@PathVariable Long id) {
                return ResponseEntity.ok(attendanceService.getAttendanceAdjustmentHistory(id));
        }
